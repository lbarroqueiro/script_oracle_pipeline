...
FUNCTION PO_SEARCH(I_po_nbr               IN  MA_STG_ORDER.MASTER_ORDER_NO%TYPE DEFAULT NULL,
                   ...
                   I_handover_date_end    IN  VARCHAR2 DEFAULT NULL, -- new 
      ...
      FROM
           (SELECT DISTINCT
            ...
            SELECT DISTINCT
                   ...
                   d.supplier supplier,
                   d.supplier_reference supplier_reference,
...
BEGIN
  ...
  L_string_query := q'{with t_binds as
                        (select :1  bv_po_nbr,
                                ...
                                :5  bv_po_source,
                                :6  bv_handover_date,
                                :7  bv_handover_date_end,
                                :8  bv_dept_list,
                                :9  bv_class_list,
                                :10 bv_subclass_list,
                                :11 bv_business_model,
                                :12 bv_buying_group_key,
                                :13 bv_factory,
                                :14 bv_order_no,
                                :15 bv_business_model_list,
                                :16 bv_buying_group_list,
                                :17 bv_buying_subgroup_list,
                                :18 bv_buying_set_list                               
                           from dual)
                          select /*+ result_cache */ new PoSearchLhbObj(null,
                                                                  master_order_no,
                                                                  order_no,
                                                                  option_id,
                                                                  item_desc,
  ...
  if I_po_nbr is not null then
    ...
    /* L_string_query := L_string_query || q'{ and ((p.master_order_no= TO_NUMBER(b.bv_po_nbr)) or
                                          ...
                                          */
    L_string_query := L_string_query || q'{ and (p.master_order_no= TO_NUMBER(b.bv_po_nbr)) }';
    --
  end if;
  --
  --
  ...
  -- handover_date
  --
  if I_handover_date_end is not null then
    --
    L_string_query := L_string_query || q'{ and trunc(p.handover_date_end) = trunc(to_date(b.bv_handover_date_end,'DD-MM-YYYY'))
                                          }';
    --
  end if; 
  --
  ...
  --
  /*
  IF I_upload_id IS NOT NULL THEN
    --
    L_string_query := L_string_query || q'{ and p.upload_id = TO_NUMBER(bv_upload_id) }';
    --
  END IF;
  --
   --
  -- handover_date
  --
  if I_upload_create_date is not null then
    --
    L_string_query := L_string_query || q'{ and trunc(p.upload_create_date) = trunc(to_date(b.bv_upload_create_date,'DD-MM-YYYY'))
                                          }';
    --
  end if; 
  */
  ...
  loop
    ...
    for i in 1..L_PoResultTbl.count loop
      --
      pipe row(PoSearchLhbobj(L_PoResultTbl(i).ord_level,
                           ...
                           L_PoResultTbl(i).product_hierarchy,
...
RETURN PoReplenSearchLhbTbl PIPELINED AS

 --
 L_program               VARCHAR2(64) := 'REPLENISHMENT_SEARCH';
 L_error_message         VARCHAR2(2000);
 L_PoReplenSearchTbl     PoReplenSearchLhbTbl;
 L_string_query          VARCHAR2(20000);
 L_sys_refcur            SYS_REFCURSOR;
 --
BEGIN
  ...
  L_string_query := q'{with t_binds as
                            ...
                            (select :1 bv_option_id          ,                                                    
                                  :2 bv_po_status            ,
                                  :3 bv_supplier             ,      
                                  :4 bv_po_source            ,
                                  :5 bv_handover_date        ,
                                  :6 bv_handover_date_end    ,
                                  :7 bv_dept_list            ,
                                  :8 bv_class_list           ,
                                  :9 bv_subclass_list        ,
                                  :10 bv_business_model       ,
                                  :11 bv_business_model_list  ,
                                  :12 bv_buying_group_key     ,
                                  :13 bv_buying_group_list    ,
                                  :14 bv_buying_subgroup_list ,
                                  :15 bv_buying_set_list      ,
                                  :16 bv_execute              ,
                                  :17 bv_factory                                   
                                from dual)
                             select /*+ result_cache */ 
                                    new PoReplenSearchLhbObj(order_level,
                                                          parent,
                                                          item_desc,
                                                          supp_ref,
                                                          supplier,
  ...
  loop
    ...
    for i in 1..L_PoReplenSearchTbl.count loop
      --
      pipe row (PoReplenSearchLhbObj( L_PoReplenSearchTbl(i).order_level,
                                   ...
                                   L_PoReplenSearchTbl(i).buying_group_name                          
                                  ));
       --
    end loop;
    --
...
FUNCTION PLANNING_SEARCH( I_option_id            IN  MA_STG_ORDER_OPTION.OPTION_ID%TYPE DEFAULT NULL,
                          I_po_status            IN  MA_STG_ORDER.STATUS%TYPE DEFAULT NULL,                          
                           ...
                           I_execute              IN  VARCHAR2 DEFAULT 'N',
                           I_factory              IN  MA_STG_ORDER_OPTION.FACTORY%TYPE DEFAULT NULL,
                           I_upload_id            IN  MA_STG_ORDER.CREATE_ID%TYPE DEFAULT NULL,
                           I_upload_create_date   IN  DATE        
                          )
RETURN PoPlannSearchLhbTbl PIPELINED AS

 --
 L_program               VARCHAR2(64) := 'PLANNING_SEARCH';
 L_error_message         VARCHAR2(2000);
 L_PoPlannSearchTbl      PoPlannSearchLhbTbl;
 L_string_query          VARCHAR2(20000);
 L_sys_refcur            SYS_REFCURSOR;
 --
BEGIN
  ...
  L_string_query := q'{with t_binds as
                            (select :1 bv_option_id            ,
                                    ...
                                    :17 bv_factory       		  ,
                                    :18 bv_upload_id            ,
                                    :19 bv_upload_create_date    
                                from dual)
                             select /*+ result_cache */ 
                                    new PoPlannSearchLhbObj(process_id,
                                                         order_level,
                                                         order_rec_no,
                                                         po_type,
                                                         option_id,
  ...
  loop
    ...
    for i in 1..L_PoPlannSearchTbl.count loop
      --
      pipe row (PoPlannSearchLhbObj(L_PoPlannSearchTbl(i).process_id,
                                 ...
                                 L_PoPlannSearchTbl(i).first_dest_name,
                                 ...
                                 L_PoPlannSearchTbl(i).buying_group_name,
                                 L_PoPlannSearchTbl(i).sku_id,
                                 L_PoPlannSearchTbl(i).size_code,
                                 L_PoPlannSearchTbl(i).exp_delivery_date
                                 )
       );
       --
    end loop;
